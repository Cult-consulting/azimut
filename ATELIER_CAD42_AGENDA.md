# Atelier Rampe de Lancement CAD 42 - Agenda & Préparation

**Date** : À planifier (après atelier BP)
**Participants** : Igor, Baptiste, Jean-Philippe Panaget, Raphaël Garcia-Brotons
**Durée estimée** : 2h
**Objectif** : Structurer le partenariat CAD 42 comme accélérateur de lancement

---

## Contexte

**CAD 42**, co-fondé par Jean-Philippe Panaget et Raphaël Garcia-Brotons, est un éditeur de logiciels métiers BTP avec un portefeuille clients établi (Vinci, Bouygues, Spie Batignolles...).

**Point clé** : CAD 42 n'est pas structuré pour la formation (pas de certification Qualiopi, pas de vocation OF). Leur cœur de métier reste le développement et la maintenance de logiciels métiers.

**Azimut** devient donc le partenaire naturel pour assurer les formations liées aux solutions CAD 42, permettant à CAD 42 de :
- Proposer un accompagnement complet à ses clients (logiciel + formation)
- Se concentrer sur son cœur de métier (logiciels)
- Valoriser son portefeuille clients sans investir dans une structure OF

Azimut bénéficie de cette rampe de lancement pour :
1. Accéder rapidement à des clients qualifiés BTP
2. Créer des formations autour des outils CAD 42
3. Construire sa crédibilité via des références majors

---

## Agenda Détaillé

### 1. Mapping Clients CAD 42 (30 min)

**Objectif** : Identifier les clients CAD 42 à potentiel formation IA / consulting HSE

#### Template à compléter avec JP/Raph

| Client | Secteur | Relation CAD42 | Contact clé | Potentiel Formation IA | Pilote Azimut | Priorité |
|--------|---------|----------------|-------------|----------------------|---------------|----------|
| Vinci | BTP Major | Client actif | ? | Élevé | **Julien** (voir note) | 1 |
| Bouygues | BTP Major | Client actif | ? | Élevé | Igor/Équipe | 2 |
| Spie Batignolles | BTP | Client actif | ? | Élevé | Igor/Équipe | 2 |
| Eiffage | BTP Major | Prospect ? | ? | Moyen | Igor/Équipe | 3 |
| ... | | | | | | |

> **⚠️ Note Vinci** : Igor travaille pour Spiraliss (50% de son activité), qui a un contrat-cadre avec Vinci. Pour éviter tout conflit d'intérêt ou inconfort, Julien pourrait piloter l'approche commerciale Vinci côté Azimut, Igor intervenant uniquement sur la délivrance pédagogique si nécessaire.

**Questions à JP/Raph :**
- [ ] Quels clients CAD 42 ont déjà exprimé un besoin formation IA ?
- [ ] Quels clients ont des projets internationaux nécessitant HSE ?
- [ ] Quels contacts peuvent être sollicités directement ?
- [ ] Y a-t-il des appels d'offres en cours ?

---

### 2. Offre Transitoire (30 min)

**Objectif** : Définir ce que la nouvelle structure peut proposer immédiatement

#### Scénario 1 : Prestation de service
CAD 42 vend une prestation globale (logiciel + accompagnement) → Azimut délivre la partie formation

| Avantages | Inconvénients |
|-----------|---------------|
| Démarrage rapide | Marge réduite |
| Pas de besoin Qualiopi immédiat | Dépendance CAD 42 |
| Clients déjà acquis | Pas de relation directe |

*Note : CAD 42 n'étant pas certifié Qualiopi, les formations ne sont pas éligibles OPCO dans ce scénario*

#### Scénario 2 : Co-branding
"Formation par CAD 42 & Azimut"

| Avantages | Inconvénients |
|-----------|---------------|
| Visibilité Azimut | Complexité contractuelle |
| Crédibilité partagée | Partage revenus |
| Construction marque | Image floue |

#### Scénario 3 : Apport d'affaires pur
CAD 42 recommande → Azimut contracte directement

| Avantages | Inconvénients |
|-----------|---------------|
| Relation directe client | Besoin Qualiopi |
| Autonomie | Commission à payer |
| Construction marque propre | Plus lent au démarrage |

**Question** : Quel scénario pour les 6 premiers mois ?

---

### 3. Cadre de Collaboration (30 min)

**Objectif** : Poser les bases d'une collaboration gagnant-gagnant

*Note : L'objectif n'est pas de "négocier" un contrat formel à ce stade, mais d'aligner les attentes entre partenaires qui sont aussi associés.*

#### Points à discuter ensemble

| Point | Options possibles | Questions ouvertes |
|-------|-------------------|-------------------|
| **Mode de collaboration** | Apport affaires / Prestation / Partenariat souple | Quel mode convient le mieux aux deux parties ? |
| **Rémunération CAD 42** | Commission % / Forfait / Échange de services | Quel modèle est le plus simple et équitable ? |
| **Périmètre** | Clients CAD 42 uniquement / Plus large | Comment évolue-t-on dans le temps ? |
| **Durée** | Engagement moral / Contrat léger | Quel niveau de formalisme nécessaire entre associés ? |

#### Propriété intellectuelle

| Élément | Propriétaire | Utilisation |
|---------|--------------|-------------|
| Contenus formations existantes CAD42 | CAD 42 | Licence à Azimut ? |
| Nouveaux contenus co-développés | À définir | 50/50 ? |
| Marque CAD 42 | CAD 42 | Usage limité co-branding |
| Marque Azimut | Azimut | Exclusive |
| Base clients | CAD 42 | Accès via apport affaires |

**Questions juridiques à trancher :**
- [ ] Qui rédige le contrat ? (Avocat commun ou chacun le sien ?)
- [ ] Quid si conflit entre CAD 42 et Azimut ?
- [ ] Clause de sortie pour JP/Raph (ils sont aussi associés Azimut)

---

### 4. Plan de Transition Opérationnel (20 min)

**Objectif** : Définir le calendrier de montée en puissance

#### Phase 1 : Quick Wins (Mois 1-3)
*Objectif* : Premiers revenus, validation modèle

| Action | Responsable | Livrable |
|--------|-------------|----------|
| Identifier 3 clients CAD42 prioritaires | JP/Raph | Liste qualifiée |
| Adapter 1-2 formations existantes | Igor | Programmes personnalisés |
| Première vente sous-traitée | CAD42 → Azimut | 1er CA |
| Setup facturation | Baptiste | Process validé |

#### Phase 2 : Structuration (Mois 4-6)
*Objectif* : Autonomie progressive

| Action | Responsable | Livrable |
|--------|-------------|----------|
| Obtention Qualiopi | Igor | Certification |
| Catalogue propre (3 modules) | Igor | Programmes certifiés |
| Premiers clients directs | Équipe | CA hors CAD42 |
| Webinaire de lancement | JP/Raph + Igor | Visibilité |

#### Phase 3 : Croissance (Mois 7-12)
*Objectif* : Équilibre CAD42 / Prospection directe

| Action | Responsable | Livrable |
|--------|-------------|----------|
| Référencement majors BTP | Igor + Julien | Panels fournisseurs |
| 5 modules catalogue | Igor | Offre complète |
| 50% CA hors CAD42 | Équipe | Indépendance |

---

### 6. Vision Moyen/Long Terme (10 min)

**Objectif** : Aligner sur la trajectoire de développement d'Azimut au-delà du BTP

#### Roadmap de développement commercial

| Phase | Horizon | Cible | Source principale |
|-------|---------|-------|-------------------|
| **1. Rampe CAD 42** | Y1 | Clients CAD 42 (Vinci, Bouygues...) | Apport affaires JP/Raph |
| **2. BTP élargi** | Y1-Y2 | Réseau associés BTP + autres acteurs BTP | Julien, prospection directe |
| **3. Diversification** | Y2-Y3 | Industrie, agroalimentaire, autres secteurs | Réseau Baptiste, nouveaux partenaires |

#### Points à discuter
- [ ] CAD 42 a-t-il vocation à rester le partenaire principal ou à devenir un partenaire parmi d'autres ?
- [ ] Comment intégrer de nouveaux apporteurs d'affaires sectoriels (industrie, agro...) sans créer de conflit ?
- [ ] Quelle place pour les formations "non-BTP" dans la relation avec CAD 42 ?

> **Vision** : Azimut doit pouvoir grandir au-delà du BTP tout en maintenant une relation forte avec CAD 42. Le partenariat CAD 42 est un accélérateur de démarrage, pas une dépendance structurelle.

---

### 5. KPIs & Reporting (10 min)

**Objectif** : Définir les indicateurs de suivi du partenariat

| KPI | Cible M6 | Cible M12 | Responsable |
|-----|----------|-----------|-------------|
| CA généré via CAD42 | 30k€ | 50k€ | Baptiste |
| Nb clients CAD42 activés | 3 | 6 | JP/Raph |
| Nb formations délivrées | 10 | 25 | Igor |
| Satisfaction clients | >4/5 | >4.5/5 | Igor |
| % CA hors CAD42 | 20% | 50% | Baptiste |

**Reporting :**
- Mensuel : Tableau de bord partagé
- Trimestriel : Revue partenariat (Igor, Baptiste, JP, Raph)

---

## Préparation Requise

### Pour JP & Raph (CAD 42)
- [ ] Liste des 10 clients CAD42 à potentiel formation IA
- [ ] Historique des demandes formation non satisfaites
- [ ] Contacts clés chez Vinci, Bouygues, Spie
- [ ] Formations existantes pouvant être sous-traitées
- [ ] Calendrier webinaires/événements CAD42

### Pour Igor
- [ ] Capacité de délivrance (nb jours dispo)
- [ ] Réseau formateurs mobilisables rapidement
- [ ] Adaptations nécessaires sur formations existantes
- [ ] Planning disponibilité Q1/Q2 2026

### Pour Baptiste
- [ ] Draft contrat d'apport d'affaires
- [ ] Process facturation inter-sociétés
- [ ] Tableau de suivi CA par source
- [ ] Conditions TVA / sous-traitance

---

## Livrables Attendus

À l'issue de cet atelier :

1. **Mapping clients** : Tableau avec 10+ clients qualifiés et priorisés
2. **Modèle de partenariat** : Scénario choisi (sous-traitance / co-branding / apport)
3. **Termes contractuels** : Principaux points validés
4. **Calendrier Phase 1** : Actions des 3 premiers mois
5. **KPIs** : Indicateurs et cibles validés

---

## Template Compte-Rendu

```markdown
# CR Atelier Rampe de Lancement CAD 42 - [DATE]

## Participants
- Igor Cannone
- Baptiste Casnedi
- Jean-Philippe Panaget
- Raphaël Garcia-Brotons

## Décisions prises

### Mapping clients
- Clients prioritaires : ...
- Quick wins identifiés : ...

### Modèle de partenariat
- Scénario retenu : ...
- Commission : ...
- Durée : ...

### Plan Phase 1
| Action | Responsable | Échéance |
|--------|-------------|----------|
| ... | ... | ... |

### KPIs validés
- ...

## Points ouverts
- ...

## Prochaines étapes
- ...
```

---

## Annexe : Questions Fréquentes

**Q : Et si un client CAD42 veut contracter directement avec Azimut ?**
A : Commission due à CAD42 pendant la durée du contrat (à définir)

**Q : Qui gère la relation client au quotidien ?**
A : À définir selon le modèle (CAD42 si sous-traitance, Azimut si apport affaires)

**Q : Quid des formations hors scope IA/BTP ?**
A : Restent chez CAD42, pas de transfert

**Q : Comment gérer les conflits d'intérêts (JP/Raph associés des deux côtés) ?**
A : Transparence totale, décisions collégiales, clause de gestion des conflits dans le pacte

---

*Document préparatoire - À enrichir avec inputs JP/Raph*
