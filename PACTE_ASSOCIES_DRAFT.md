# Pacte d'Associés - Préparation

**Statut** : Draft - À valider en atelier BP
**Date** : 30 janvier 2026

---

## Contexte

Le pacte d'associés complète les statuts et régit les relations entre associés. Il est **confidentiel** (contrairement aux statuts déposés au greffe).

### Tour de table prévu

| Associé | Parts | % estimé | Catégorie | Engagement |
|---------|-------|----------|-----------|------------|
| Igor Cannone | 2 | ~33% | Actif - Président | Majoritaire |
| Baptiste Casnedi | 1 | ~17% | Actif - DG | Partiel |
| Julien Gardette | 1 | ~17% | Actif | Partiel |
| Éloïse Beauvais | 1 | ~17% | Actif | Partiel |
| JP Panaget + Raph Garcia-Brotons | 1 (0.5+0.5) | ~17% | Passifs | Apport affaires |

*Note : % à confirmer lors de l'atelier BP*

---

## Clauses Essentielles

### 1. Gouvernance

#### 1.1 Organes de décision

| Décision | Majorité requise | Commentaire |
|----------|------------------|-------------|
| **Opérations courantes - Palier 1** | Président seul | < seuil 1 (ex: 2k€) |
| **Opérations courantes - Palier 2** | Président + DG | Entre seuil 1 et seuil 2 (ex: 2k€ - 10k€) |
| Investissements significatifs | Majorité simple | > seuil 2 |
| Modification statuts | Unanimité ou 2/3 | À définir |
| Entrée nouvel associé | Unanimité | Protection dilution |
| Cession de la société | Unanimité | Protection tous |
| Exclusion d'un associé | Majorité qualifiée (hors concerné) | Cas graves |

**Questions à valider :**
- [ ] Seuil palier 1 - Président seul (1k€ ? 2k€ ? 3k€ ?)
- [ ] Seuil palier 2 - Président + DG (5k€ ? 10k€ ? 15k€ ?)
- [ ] Majorité pour décisions stratégiques (2/3 ? 75% ?)
- [ ] Droit de veto pour certains sujets ?

#### 1.2 Réunions

| Type | Fréquence | Participants | Quorum |
|------|-----------|--------------|--------|
| Comité opérationnel | Mensuel | Associés actifs | 3/4 |
| AG ordinaire | Annuelle | Tous | Tous convoqués |
| AG extraordinaire | Sur demande | Tous | 2/3 présents |

---

### 2. Engagement des Associés

#### 2.1 Engagement temps (associés actifs)

*Note : Unité flexible selon les profils - h/semaine ou j/mois selon la nature de l'engagement*

| Associé | Engagement minimum | Unité | Activités autorisées en parallèle |
|---------|-------------------|-------|-----------------------------------|
| Igor | ? | h/sem ou j/mois | Spiraliss (50%), autres formations |
| Baptiste | ? | h/sem ou j/mois | Autres activités entrepreneuriales |
| Julien | ? | j/mois | Activité BTP principale |
| Éloïse | ? | h/sem ou j/mois | Autres clients communication |

**Proposition d'unités par profil :**
- **Engagement régulier** (Igor, Baptiste) → h/semaine (ex: 8h, 16h, 24h)
- **Engagement ponctuel** (Julien, Éloïse) → j/mois (ex: 2j, 4j, 6j)

**Questions à valider :**
- [ ] Unité par associé (h/semaine vs j/mois) ?
- [ ] Engagement minimum pour chaque associé actif ?
- [ ] Flexibilité en fonction du CA généré ?
- [ ] Clause de "best efforts" plutôt que temps fixe ?
- [ ] Période de référence pour le suivi (mensuel ? trimestriel ?)

#### 2.2 Associés passifs (JP + Raph)

| Obligation | Description |
|------------|-------------|
| Apport affaires | Mise en relation clients CAD 42 |
| Non-obstruction | Ne pas bloquer le développement |
| Information | Droit d'information trimestriel |
| Pas d'engagement temps | Aucune obligation opérationnelle |

---

### 3. Non-Concurrence

#### 3.1 Périmètre

| Élément | Proposition | À valider |
|---------|-------------|-----------|
| **Activités interdites** | Formation IA/Lean/HSE pour le BTP | Trop restrictif ? |
| **Zone géographique** | France métropolitaine | International ? |
| **Durée pendant association** | Toute la durée | Standard |
| **Durée après sortie** | 2 ans | 1 an ? 3 ans ? |
| **Exceptions** | Activités déclarées préexistantes | À lister |

#### 3.2 Exceptions pré-déclarées

| Associé | Activité autorisée | Lien potentiel |
|---------|-------------------|----------------|
| Igor | Spiraliss (Vinci) | Conflit géré via Julien sur Vinci |
| JP + Raph | CAD 42 | Partenariat structuré |
| Baptiste | Activités tech hors formation | Pas de conflit |
| Julien | Activité BTP (hors formation) | Pas de conflit |
| Éloïse | Autres clients communication | Pas de conflit |

**Questions à valider :**
- [ ] Liste exhaustive des exceptions ?
- [ ] Procédure de déclaration de nouvelle activité ?
- [ ] Sanction en cas de violation ?

---

### 4. Cession de Parts

#### 4.1 Clause d'agrément

> Toute cession de parts à un tiers doit être approuvée par les associés restants.

| Cession | Agrément requis | Majorité |
|---------|-----------------|----------|
| À un associé existant | Non | - |
| À un conjoint/héritier | Oui (simplifié) | Majorité simple |
| À un tiers externe | Oui | Unanimité ou 2/3 |

#### 4.2 Droit de préemption

> En cas de cession, les autres associés ont un droit prioritaire d'achat.

| Étape | Délai | Action |
|-------|-------|--------|
| 1. Notification | J | Le cédant notifie son intention |
| 2. Réponse associés | J+30 | Les associés indiquent s'ils préemptent |
| 3. Exercice préemption | J+60 | Signature de la cession |
| 4. Cession au tiers | J+90 | Si pas de préemption |

#### 4.3 Tag-Along (droit de sortie conjointe)

> Si un associé majoritaire vend, les minoritaires peuvent vendre aux mêmes conditions.

**Application** : Si Igor (33%) vend, les autres peuvent exiger de vendre aussi.

#### 4.4 Drag-Along (obligation de sortie conjointe)

> Si X% des associés acceptent une offre, les autres doivent suivre.

**Seuil proposé** : 75% ou 80%

**Questions à valider :**
- [ ] Seuil drag-along (75% ? 80% ?)
- [ ] Délai de préemption (30j ? 60j ?)

---

### 5. Valorisation

#### 5.1 Méthode de valorisation

| Méthode | Description | Quand l'utiliser |
|---------|-------------|------------------|
| **Multiple EBITDA** | X fois l'EBITDA moyen 3 ans | Cession volontaire |
| **Actif net comptable** | Capitaux propres | Bad leaver |
| **Expert indépendant** | Valorisation par tiers | Désaccord |

**Proposition** :
- Good leaver : Multiple EBITDA (4-6x) ou valeur marché
- Bad leaver : Actif net comptable (décote)

#### 5.2 Good Leaver / Bad Leaver

| Situation | Catégorie | Valorisation |
|-----------|-----------|--------------|
| Départ volontaire (préavis respecté) | Good | 100% valeur |
| Départ pour raison personnelle/santé | Good | 100% valeur |
| Décès | Good | 100% valeur (héritiers) |
| Révocation pour faute grave | Bad | 50-70% valeur ou ANR |
| Violation non-concurrence | Bad | 50% valeur ou ANR |
| Démission sans préavis | Bad | 70% valeur |

**Questions à valider :**
- [ ] Multiple EBITDA (4x ? 5x ? 6x ?)
- [ ] Décote bad leaver (30% ? 50% ?)
- [ ] Définition "faute grave" ?

---

### 6. Incessibilité Temporaire (Lock-up)

| Période | Durée | Exception |
|---------|-------|-----------|
| Lock-up initial | 3 ans | Décès, invalidité |
| Lock-up après augmentation capital | 1 an | Idem |

**Questions à valider :**
- [ ] Durée lock-up (2 ans ? 3 ans ?)

---

### 7. Propriété Intellectuelle

#### 7.1 Contenus créés

| Contenu | Propriétaire | Conditions |
|---------|--------------|------------|
| Formations créées pour Azimut | Azimut | 100% propriété société |
| Méthodologies développées | Azimut | Licence perpétuelle si départ |
| Contenus préexistants | Créateur original | Licence à Azimut |
| Contenus CAD 42 adaptés | Voir accord CAD 42 | À définir |

#### 7.2 En cas de départ

| Situation | Droits sur la PI |
|-----------|------------------|
| Good leaver | Licence d'usage limitée (non-commerciale) |
| Bad leaver | Aucun droit |
| Cession totale société | Transfert avec la société |

---

### 8. Confidentialité

| Élément | Durée | Sanction |
|---------|-------|----------|
| Informations commerciales | Illimitée | Dommages + exclusion |
| Méthodes pédagogiques | 5 ans après départ | Dommages |
| Données clients | Illimitée | Dommages + RGPD |
| Termes du pacte | Illimitée | - |

---

### 9. Gestion des Conflits d'Intérêts

#### 9.1 Cas spécifique JP + Raph (CAD 42)

| Situation | Règle |
|-----------|-------|
| Décision impactant CAD 42 | JP/Raph s'abstiennent du vote |
| Négociation commerciale avec CAD 42 | Mandataire non directement lié à CAD 42 (DG ou associé désigné, pas le Président) |
| Litige avec CAD 42 | Médiation externe obligatoire |

#### 9.2 Cas spécifique Président (Spiraliss/Vinci)

| Situation | Règle |
|-----------|-------|
| Prospection Vinci | Associé non lié à Spiraliss pilote (pas le Président) |
| Délivrance formation Vinci | Président peut délivrer (pas de conflit sur l'exécution) |
| Conflit Spiraliss/Azimut | Déclaration préalable obligatoire au Comité |

#### 9.3 Procédure générale

1. **Déclaration** : Tout associé déclare un conflit potentiel
2. **Abstention** : L'associé concerné s'abstient du vote
3. **Registre** : Les conflits sont consignés dans un registre
4. **Médiation** : En cas de litige, médiation avant contentieux

---

### 10. Résolution des Litiges

| Étape | Description | Délai |
|-------|-------------|-------|
| 1. Discussion amiable | Entre associés concernés | 15 jours |
| 2. Médiation | Médiateur agréé (CMAP, etc.) | 30 jours |
| 3. Arbitrage ou Tribunal | Si échec médiation | - |

**Proposition** : Clause compromissoire (arbitrage) plutôt que tribunal

---

### 11. Clauses Spécifiques Azimut

#### 11.1 Relation CAD 42

| Clause | Description |
|--------|-------------|
| Priorité apport affaires | JP/Raph s'engagent à orienter clients CAD 42 vers Azimut |
| Non-exclusivité | Azimut peut développer hors CAD 42 |
| Durée engagement | 3 ans minimum, puis tacite reconduction |
| Commission | À définir dans accord séparé (pas dans pacte) |

#### 11.2 Évolution du capital

| Événement | Règle |
|-----------|-------|
| Augmentation capital | Droit préférentiel de souscription proportionnel |
| Dilution acceptée | Si nouvel associé stratégique (vote unanime) |
| BSPCE/Stock-options futurs | Pool de 10% réservé (formateurs clés) |

---

## Checklist Validation

### Clauses à valider en atelier BP

**Gouvernance**
- [ ] Seuil palier 1 (Président seul) : _____ €
- [ ] Seuil palier 2 (Président + DG) : _____ €
- [ ] Majorité décisions stratégiques : _____
- [ ] Fréquence comité opérationnel : _____

**Engagement**
- [ ] Engagement temps Igor : _____ (h/sem ou j/mois)
- [ ] Engagement temps Baptiste : _____ (h/sem ou j/mois)
- [ ] Engagement temps Julien : _____ j/mois
- [ ] Engagement temps Éloïse : _____ (h/sem ou j/mois)

**Sortie**
- [ ] Durée lock-up : _____ ans
- [ ] Délai préemption : _____ jours
- [ ] Seuil drag-along : _____%

**Valorisation**
- [ ] Multiple EBITDA : _____x
- [ ] Décote bad leaver : _____%

**Non-concurrence**
- [ ] Durée après sortie : _____ ans
- [ ] Zone géographique : _____

---

## Prochaines Étapes

1. 🔲 **Validation clauses** - Atelier BP (semaine du 3 février)
2. 🔲 **Rédaction draft** - Avocat ou modèle adapté
3. 🔲 **Relecture collective** - Tous les associés
4. 🔲 **Signature** - Avant création juridique

---

## Ressources

- **Modèles** : LegalStart, Captain Contrat, avocat spécialisé
- **Coût estimé** :
  - Modèle adapté : 500-1 500€
  - Avocat sur-mesure : 2 000-5 000€
- **Recommandation** : Modèle adapté + relecture avocat (1 500-2 500€)

---

*Document préparatoire - À compléter lors de l'atelier BP*
